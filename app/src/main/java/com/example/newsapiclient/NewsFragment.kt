...
class NewsFragment : Fragment() {
    ...
    private val onScrollListener = object : OnScrollListener(){
        ...
        override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
            ...
            var t = 0
            if (shouldPaginate) t = 1
            else t=2
                //     Toast.makeText(activity, "Scrolled : $t", Toast.LENGTH_LONG)
            if(shouldPaginate){
                page++
                viewModel.getNewsHeadLines(country,page)
                isScrolling = false
                Toast.makeText(activity, "New page : $page", Toast.LENGTH_LONG).show()
            }
